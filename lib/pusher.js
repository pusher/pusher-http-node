...
Pusher.prototype.auth = function(socketId, channel, channelData) {
  ...
  var signature = crypto.createHmac('sha256', this.options.secret)
    .update(stringToSign)
    .digest('hex');
  returnHash.auth = this.options.key + ':' + signature;
  return(returnHash);
};
...
Pusher.prototype.post = function(options, callback) {
  ...
  var url = this.scheme + '://' + this.domain + port + path + "?" + queryString;
