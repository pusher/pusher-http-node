...
const validateUserData = function (userData) {
  ...
  if (Object.prototype.hasOwnProperty.call(userData, "watchlist")) {
    validateWatchlist(userData.watchlist)
  }
}

const validateWatchlist = function (watchlist) {
  if (!Array.isArray(watchlist)) {
    throw new Error("Watchlist must be an array: '" + watchlist + "'")
  }
  if (watchlist.length > 0) {
    watchlist.forEach((watchId) => {
      validateUserId(watchId)
    })
  }
}
