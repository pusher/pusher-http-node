...
var validateEventName = function(name) {
  ...
  if (name.length > 200) {
    throw new Error("Too long event name: '" + name + "'");
  }
}
 ...
 *       "event: "...", # event name
 *       "data": "...", # data, make 10k
 *       "socket_id": null, # optional
 *     }
 *
 * @params {Object[]} array of events to send.
 * @params {requestCallback} [callback]
 */
Pusher.prototype.triggerBatch = function(events_, callback) {
  if (!(events_ instanceof Array)) {
    throw new Error("events must be an instance of array");
  }
  events.triggerBatch(
    this,
    events_.map(function(e) {
      validateChannel(e.channel);
      if (e.socket_id) {
        validateSocketId(e.socket_id);
      }
