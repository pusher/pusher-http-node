...
describe("NativeNotificationClient", function() {
  ...
  // TODO enable this test if you can figure out how to use this nock thing.
  // I just can't figure it out.
  // https://github.com/node-nock/nock/issues/640
  xit("should have X-Pusher-Library header", function(done){
    var headers;

    var mock = nock("http://nativepush-cluster1.pusher.com:80")
      .post("/server_api/v1/apps/1234/notifications")
    ...
    client.notify(['yolo'],{
      ...
      console.log("DONE");
      expect(headers).to.have.key('X-Pusher-Library');
      expect(mock.isDone()).to.be(true);
      done();
    });
  });
